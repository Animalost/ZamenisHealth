...
namespace Persistence.CXN.Interfaces
{
    public interface IFacturacion
    {
        ...
        bool updateDE(CXN_FACTURA F);
        List<Class_DetailDE> getDetailDE(int Factura);
        bool Anula_Factura(CXN_FACTURA F);
        CXN_FACTURA GetFacElectronica(CXN_FACTURA F);
        List<FacturasR> Fac_ExportRec(int Numero_Fac, int cia, string Tipo);
        List<string> getHomologos(DateTime Desde, DateTime Hasta, int Cia, int Ase);
        Dictionary<int, string> getNumFacs(DateTime Desde, DateTime Hasta, int Cia, int Ase);
        ...
        List<CXN_FACTURA> getFacsParticulares(DateTime Fecha, int Cia);
        ...
        List<CXN_FACTURA> GetOtrasFacturasForConvertElectron(int Cia, DateTime Desde, DateTime Hasta);
