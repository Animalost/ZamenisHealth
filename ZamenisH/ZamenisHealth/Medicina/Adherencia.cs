...
namespace ZamenisHealth.Medicina
{
    ...
    {               
        ...
        void Encabezados()
        {
            ...
            dt = new DataTable();
            ChexkSend = dt.Columns.Add("Elegir", typeof(bool));
            Aposito = dt.Columns.Add("Aposito", typeof(string));
        }
        private void Adherencia_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                List<CXN_ADHERENCIA> lista = rpositorioAdherencia.listaApositos();
                if (lista != null)
                {
                    Encabezados();
        ...
        void Estilos(DataGridView D, DataTable t)
        {
            ...
            D.Columns["Elegir"].Width = 100;

            D.ReadOnly = false;
            D.Columns["Elegir"].ReadOnly = false;
            D.Columns["Aposito"].ReadOnly = true;
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    object valor = row.Cells["Elegir"].Value;
                    bool isChecked = valor != null && Convert.ToBoolean(valor);

                    if (isChecked)
                    {
                        string Seleccion = row.Cells["Aposito"].Value.ToString();
                        string desc = rpositorioAdherencia.DescripcionAposito(Seleccion);
                        if (desc != "")
                        {
                            Ap.Add(Seleccion + " -- Agregar descripcion aqui -- ");
                            Imprimir = Imprimir + 
                                       Seleccion + 
