...
{
    public partial class InitializeClock : Form
    {
        int Iniciar = 60;
        ...
        private void InitializeClock_Load(object sender, EventArgs e)
        {
            ActivaTimer();
        }
        ...
        private void IniciarReloj()
        {
            try
            {
                ...
                path.AddEllipse(13, 12, 150, 150);
                Region regPath = new Region(path);
                pictureBox1.Region = regPath;
                path.Dispose();

                bmpS = new Bitmap(Properties.Resources2.Secundario);
                bmpM = new Bitmap(Properties.Resources2.MinuteroZH);
                bmpH = new Bitmap(Properties.Resources2.HorarioZH);
            }
            catch (Exception ex)
            {
                timer1.Enabled = false;
                pictureBox1.Visible = false;
                MessageBox.Show("Error en reloj analogico", "Se cerrara", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }
